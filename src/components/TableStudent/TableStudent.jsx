...
const TableStudent = () => {
  ...
  function GetTrips() {
    axiosInstance
      .get("student?page=1&pageSize=100")
      .then((response) => {
        setStudent(response.data.items); // استخدم items للوصول إلى قائمة الطلاب
        ...
        <tbody>
          {loading ? (
            ...
            student.map((item, index) => (
              <tr key={index} className="border-b border-gray-700 bg-gray-800 hover:bg-gray-600 text-white">
                <td className="px-6 py-4 text-right">{item.major}</td>
                <th scope="row" className="whitespace-nowrap px-6 py-4 font-medium text-right">
                  {item.name}   
                </th>
              </tr>
            ))
